...
const countAppointmentsForDoctor = (doctorId) => {
  ...
  return appointments.filter((appointment) => appointment.doctorId._id === doctorId).length;
};

const MAX_APPOINTMENTS_PER_DOCTOR = 3;

const isAppointmentLimitReached = () => {
  const doctorAppointmentsCount = countAppointmentsForDoctor(ele?.userId?._id);
